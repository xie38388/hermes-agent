...
"""
Tool Loop Mixin - Manages tool execution, concurrency, and result processing.
Extracted from run_agent.py to isolate tool dispatch from agent logic.

This mixin provides:
  - Sequential and concurrent tool execution
  - Tool call deduplication and validation
  - Delegate task capping
  - Tool invocation with callbacks

It does NOT own:
  - Tool definitions (stays in tools/ registry)
  - LLM interaction (stays in LLMClientMixin)
  - Context management
"""
import asyncio
...
class ToolLoopMixin:
    """Mixin providing tool execution loop and dispatch.

    Expects the host class to provide:
      - self.valid_tool_names: set of registered tool names
      - self.session_id: current session identifier
      - self._interrupt_requested: bool flag for cancellation
      - self.tool_start_callback / tool_complete_callback / tool_progress_callback
      - self.clarify_callback
      - self._memory_manager / self._memory_store
      - self._session_db / self._todo_store / self._checkpoint_mgr
