...
if TYPE_CHECKING:
    pass
"""
Conversation Mixin - Contains the main run_conversation loop, model switching,
session state management, and the chat convenience method.

Extracted from run_agent.py Step 6 of modular refactoring.
This is the largest single extraction (~2900 lines) and contains:
  - run_conversation(): The main agent loop (API call → response → tool exec → repeat)
  - reset_session_state(): Token counter reset for fresh sessions
  - switch_model(): In-place model/provider swap
  - chat(): Simple convenience wrapper
"""
import copy
import hashlib
import json
...
from agent.shared_utils import _sanitize_surrogates, _install_safe_stdio
from agent.shared_utils import IterationBudget
...
from agent.model_metadata import (
    save_context_length, get_next_probe_tier,
    parse_context_limit_from_error, parse_available_output_tokens_from_error,
)
from agent.memory_manager import build_memory_context_block
from agent.shared_utils import _sanitize_messages_surrogates, _sanitize_messages_non_ascii
from utils import env_var_enabled



class ConversationMixin:
    """Mixin providing the main conversation loop and related methods."""
